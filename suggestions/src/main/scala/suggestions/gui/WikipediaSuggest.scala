...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val suggestions: Observable[Try[List[String]]] = {
      searchTerms.flatMap(s => wikiSuggestResponseStream(s).timedOut(1L).recovered)
    }
    ...
    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.flatMap(s => wikiPageResponseStream(s).recovered)
